...
public struct CinemachineInputAxisDriver
{
    ...
    public float Update(float deltaTime, ref AxisState axis)
    {
        if(axisSpace ==AxisSpace.x)
        {
            ...
            { 
                inputValue=0;
                inputValue += UserInput.Instance.ControllerCursorDeltaPosition.x * Time.unscaledDeltaTime; 
                inputValue += UserInput.Instance.MouseCursorDeltaPosition.x; 
            }
        ...
        else
        {
            ...
            { 
                inputValue=0;
                inputValue += UserInput.Instance.ControllerCursorDeltaPosition.y * Time.unscaledDeltaTime;
                inputValue += UserInput.Instance.MouseCursorDeltaPosition.y;
            }
            catch (ArgumentException) {}       
        }
